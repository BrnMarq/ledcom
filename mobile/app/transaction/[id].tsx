  ...
  return (
    <>
      ...
      <ScrollView className="flex-1 bg-gray-50">
        {/* Header Summary */}
        <View
          className={`p-8 pb-12 items-center relative ${transaction.flow === "IN" ? "bg-emerald-500" : "bg-red-500"}`}
        >
          <TouchableOpacity
        ...
        <Text className="text-white/80 font-bold uppercase tracking-widest text-xs mb-2 mt-4">
          Monto Total
        </Text>
        <Text className="text-white text-5xl font-black">
          {transaction.flow === "IN" ? "+" : "-"}
          {formatCurrency(transaction.totalValue, transaction.account?.symbol)}
        </Text>
        <View className="bg-white/20 px-3 py-1 rounded-full mt-4">
          <Text className="text-white font-bold text-xs">
            {transaction.account?.symbol || "USD"}
          </Text>
        </View>
      </View>
      ...
      <View className="px-4 -mt-6">
        ...
        <View className="bg-white rounded-3xl p-6 shadow-sm mb-6">
          <View className="flex-row items-center mb-6">
            <View
              className={`p-3 rounded-2xl mr-4 ${transaction.flow === "IN" ? "bg-emerald-100" : "bg-red-100"}`}
            >
              {transaction.flow === "IN" ? (
          ...
          <View className="space-y-4">
            <View className="flex-row items-center">
              ...
              <Text className="text-gray-600 ml-3 text-sm flex-1 capitalize">
        ...
        {/* AI Context Card */}
        <View className="bg-white rounded-3xl p-6 shadow-sm mb-6">
          <Text className="text-gray-400 text-[10px] font-bold uppercase mb-3 tracking-widest">
            Resumen IA
          </Text>
          <Text className="text-gray-800 leading-6 italic">
            "{transaction.context || "Sin descripción disponible."}"
          </Text>
        </View>
        ...
        {transaction.items && transaction.items.length > 0 && (
          <View className="bg-white rounded-3xl p-6 shadow-sm mb-10">
            <View className="flex-row items-center mb-4">
              ...
              <Text className="text-gray-800 font-black text-lg ml-2">
                Artículos
              </Text>
            </View>
            ...
            {transaction.items.map((item, index) => (
              <View
                key={item.id}
                className={`py-4 flex-row justify-between items-center ${index !== transaction.items.length - 1 ? "border-b border-gray-50" : ""}`}
              >
                <View className="flex-1">
                ...
                <Text className="text-gray-900 font-black">
                  {formatCurrency(item.totalPrice, transaction.account?.symbol)}
                </Text>
              </View>
            ))}
          </View>
